...
class Toutiao:
    ...
    def get_recommended_content(self):
        ...
        try:
            ...
            # 检查是否正确进入首页
            if "头条" not in driver.title:
                print("警告：可能未正确进入首页，尝试刷新页面")
                driver.refresh()
                time.sleep(5)
            ...
            # 方法1: 直接查找所有内容链接
            try:
                print("尝试方法1: 查找所有文章和视频链接")
                links = driver.find_elements(By.TAG_NAME, 'a')
                print(f"找到 {len(links)} 个a标签")
                
                for link in links:
                    try:
                        href = link.get_attribute('href')
                        if href:
                            if ('/article/' in href or '/video/' in href or '/group/' in href) and 'toutiao.com' in href:
                                if href not in content_links:
                                    content_links.append(href)
                                    print(f"找到内容链接: {href}")
                    except Exception as e:
                        continue
                        
                print(f"方法1共找到 {len(content_links)} 个有效链接")
            except Exception as e:
                print(f"方法1失败: {e}")
            ...
            if len(content_links) < 5:
                try:
                    print("尝试方法2: 通过文章卡片提取链接")
                    # 尝试多种卡片选择器
                    card_selectors = [
                        "//div[contains(@class, 'ArticleCard')]",
                        "//div[contains(@class, 'feed-card')]", 
                        "//div[contains(@data-log, 'article')]",
                        "//div[contains(@class, 'content-card')]",
                        "//div[contains(@class, 'single-mode')]",
                        "//div[contains(@class, 'multi-mode')]"
                    ]
                        ...
                        try:
                            ...
                            for card in cards:
                                try:
                                    ...
                                    href = link.get_attribute('href')
                                    if href and ('/article/' in href or '/video/' in href or '/group/' in href) and href not in content_links:
                                        content_links.append(href)
                                except:
                                    continue
                        except:
                    ...
                    print(f"方法2后共找到 {len(content_links)} 个有效链接")
                except Exception as e:
                    print(f"方法2失败: {e}")
            ...
            if len(content_links) < 5:
                try:
                    print("尝试方法3: 使用XPath直接匹配链接")
                    xpath_patterns = [
                        "//a[contains(@href, '/article/')]",
                        "//a[contains(@href, '/video/')]",
                        "//a[contains(@href, 'toutiao.com/group/')]"
                    ]
                    ...
                    for pattern in xpath_patterns:
                        try:
                            ...
                            for link in links:
                                href = link.get_attribute('href')
                                if href and href not in content_links:
                                    content_links.append(href)
                        except:
                            continue
                            
                    print(f"方法3后共找到 {len(content_links)} 个有效链接")
                except Exception as e:
                    print(f"方法3失败: {e}")
            
            # 去重并限制数量
            content_links = list(set(content_links))[:20]
            print(f"最终获取到 {len(content_links)} 个推荐内容链接")
    ...
    def _process_user_page(self, link, driver, original_window, log_like_operation):
        ...
        try:
            ...
            # 对视频链接执行点赞操作
            from utils.config import config
            like_rate = config.get("operation.like_rate", 100)
            comment_rate = config.get("operation.comment_rate", 100)
            ...
            for i, video_link in enumerate(video_links):
                try:
                    ...
                    if should_like:
                        ...
                        print(f"跳过点赞（点赞率: {like_rate}%）")
                    
                    # 根据概率执行评论操作
                    should_comment = random.randint(1, 100) <= comment_rate
                    if should_comment and self.match_comment_item_map:
                        try:
                            print("尝试执行视频评论操作...")
                            page_title = driver.title
